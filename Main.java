...
public class Main extends Application {
    ...
    public void launch() {
        ...
        if (verticalTiles > 1 && verticalTiles < 31 && horizontalTiles > 1 && horizontalTiles < 41 && minesNeeded > 0 && minesNeeded <= (horizontalTiles*verticalTiles-1)) {
            game.setRoot(Minesweeper.createContent());
        }else{
            Alert alert = new Alert(Alert.AlertType.NONE, "Height must be greater than 1 and less than 31; width must be greater than 1 and less than 41; there must be at least one mine.", ButtonType.OK);
            alert.showAndWait();
            if (alert.getResult() == ButtonType.OK) {
                return;
            }
